    ...
    public void start() {
        ...
        do {
            String input = view.getInputData(
                    "Введите данные через пробел (Фамилию Имя Отчество Дату рождения Номер телефона Пол),\n" +
            ...
            if (input.equals("Exit")) {
                ...
                if (inputDataCount == -1) {
                    view.printOutputData("Слишком мало данных на вводе (должно быть " + DataCheck.dataCount
                            + " разделённых пробелом ' ': Фамилия Имя Отчество Номер Телефона Дата Рождения Пол)\n");
